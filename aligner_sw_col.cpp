	...
	RandomSource& rand)    // pseudo-random generator
{
	...
	while((int)row >= 0) {
		...
		ASSERT_ONLY(AlnScore bestThisRound = tab[row][tabcol].best[curC]);
	...
	for(int to = 0; to < 4; to++) {
		...
		for(int fr = 0; fr < 4; fr++) {
			...
			if(frMask.refExtendPossible()) {
				// Extend is possible
				frBest.score_ -= pen_->refGapExtend();
				assert_leq(frBest.score_, 0);
				if(frBest.score_ >= minsc_ && frBest >= myBest) {
					if(frBest > myBest) {
						myBest = frBest;
						myMask.clear();
					}
	...
	for(size_t row = 1; row <= rdf_ - rdi_; row++) {
		...
		bool onlyDiagInto =
			(row+1             <= (size_t)pa_->gapBar ||
			 rdf_ - rdi_ - row <= (size_t)pa_->gapBar);
		...
		if(cur.finalize(minsc_)) {
			UPDATE_SOLS(tab[row][col], row, col);
			validInRow = true;
		}
