...
/* Shift 16 8-bit unsigned chars right logical immediate */
VECLIB_INLINE __m128i vec_shiftrightimmediate16ub (__m128i v, intlit8 count)
{
  if ((unsigned long) count >= 16) {
    /* SSE2 shifts >= element_size or < 0 produce 0; Altivec/MMX shifts by count%element_size. */
    return (__m128i) vec_splats (0);
...
/* Compare 16 8-bit unsigned bytes for < to vector mask */
VECLIB_INLINE __m128i vec_comparelt16ub (__m128i left, __m128i right)
{ return (__m128i) vec_cmplt ((vector unsigned char) left, (vector unsigned char) right); }
