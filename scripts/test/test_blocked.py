...
for block_sz in [400, 1600, 6000, 12000]:
    for read_len in [30, 50, 100]:
        ...
        cmd = './bowtie2 --no-hd --reorder -p 8 --reads-per-block %d --block-bytes %d -x example/index/lambda_virus -1 %s -2 %s -S %s' % (reads_per_block, block_sz, ofn1, ofn2, osamfn)
